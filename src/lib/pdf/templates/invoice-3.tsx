...
const s = StyleSheet.create({
  ...
  // ── Main content row: customer (left) | doc meta (right) ─────────────────
  mainRow: {
    flexDirection: "row",
    justifyContent: "space-between",
    paddingBottom: 16,
...
export function InvoicePdf3({ data }: { data: PdfTemplateData }) {
  const notes = stripHtml(data.notes);

  return (
    <Document title={`Invoice ${data.number}`} author={data.organizationName}>
      <Page size="A4" style={s.page}>
        ...
        {/* Main content: customer (left) | invoice meta (right) */}
        <View style={s.mainRow}>
          ...
          <View style={s.docMetaBlock}>
            ...
            {data.secondaryDate ? (
              <View style={s.metaRow}>
                <Text style={s.metaLabel}>{data.secondaryDateLabel ?? "Due Date"}</Text>
        ...
        <View style={s.footer} fixed>
          <Text style={s.footerText}>{data.organizationName} · Invoice {data.number}</Text>
